...
@dataclass(frozen=True, slots=True)
class ConfirmFactsCommand:
    case_id: str
    profile_version: int
    fact_proposals: list[FactInput]
    confirmed_fact_keys: list[str]
    rejected_fact_keys: list[str]
    request_id: str
...
class CasesService:
    ...
    async def confirm_facts(self, command: ConfirmFactsCommand, principal: Principal) -> CaseDetail:
        _require_cases_write(principal)
        decisions = validate_fact_decisions(
            command.fact_proposals,
            confirmed_fact_keys=command.confirmed_fact_keys,
            rejected_fact_keys=command.rejected_fact_keys,
        )
        now = datetime.now(UTC)
        async with self._uow_factory() as uow:
            repository = _repository(uow)
            case = await repository.lock_case(command.case_id, principal.org_id)
            if case is None or not _can_confirm_facts(case, principal):
                raise DomainError(code="RESOURCE_NOT_FOUND", message="事项不存在或无权确认事实")
            if case.current_profile_version != command.profile_version:
                raise DomainError(
                    code="RESOURCE_VERSION_CONFLICT",
                    message="事项画像已更新, 请刷新后重试",
                )
            previous = await repository.get_profile(case.id, case.org_id, command.profile_version)
            if previous is None:
                raise DomainError(code="RESOURCE_CONFLICT", message="待确认的画像版本不存在")
            previous_facts = await repository.list_facts(
                case.id, case.org_id, command.profile_version
            )
            new_version = previous.profile_version + 1
            profile = replace(
                previous,
            ...
            facts = build_confirmed_fact_snapshot(
                case=case,
                previous_facts=previous_facts,
                fact_proposals=command.fact_proposals,
                decisions=decisions,
                profile_version=new_version,
                reviewer_decision=bool(principal.roles.intersection({"reviewer", "org_admin"})),
            )
            await repository.create_profile(profile, actor_id=principal.user_id)
            await repository.flush()
            await repository.create_facts(facts, actor_id=principal.user_id, occurred_at=now)
            ...
            await repository.create_audit_log(
                org_id=case.org_id,
                actor_user_id=principal.user_id,
                action_code="case.facts.confirmed",
                resource_id=case.id,
                request_id=command.request_id,
                occurred_at=now,
            )
...
def _can_confirm_facts(case: CaseRecord, principal: Principal) -> bool:
    return case.owner_user_id == principal.user_id or bool(
        principal.roles.intersection({"reviewer", "org_admin"})
    )
...
def validate_fact_decisions(
    fact_proposals: list[FactInput],
    *,
    confirmed_fact_keys: list[str],
    rejected_fact_keys: list[str],
) -> dict[str, str]:
    proposal_keys = [fact.fact_key for fact in fact_proposals]
    if len(proposal_keys) != len(set(proposal_keys)):
        raise DomainError(code="VALIDATION_FAILED", message="事实候选键不能重复")
    confirmed = set(confirmed_fact_keys)
    rejected = set(rejected_fact_keys)
    if len(confirmed) != len(confirmed_fact_keys) or len(rejected) != len(rejected_fact_keys):
        raise DomainError(code="VALIDATION_FAILED", message="事实决定键不能重复")
    if confirmed.intersection(rejected):
        raise DomainError(code="VALIDATION_FAILED", message="同一事实不能同时确认和拒绝")
    if set(proposal_keys) != confirmed.union(rejected):
        raise DomainError(
            code="VALIDATION_FAILED",
            message="每个事实候选都必须明确确认或拒绝",
        )
    for fact in fact_proposals:
        ...
        reject_restricted_identifiers(fact.value)
    return {
        fact_key: "confirmed" if fact_key in confirmed else "rejected" for fact_key in proposal_keys
    }
...
def build_confirmed_fact_snapshot(
    *,
    case: CaseRecord,
    previous_facts: list[CaseFactRecord],
    fact_proposals: list[FactInput],
    decisions: dict[str, str],
    profile_version: int,
    reviewer_decision: bool,
) -> list[CaseFactRecord]:
    confirmed_proposal_keys = {
        fact_key for fact_key, decision in decisions.items() if decision == "confirmed"
    }
    snapshot = [
        replace(
            fact,
        ...
        if fact.confirmation_status == "confirmed" and fact.fact_key not in confirmed_proposal_keys
    ]
    source_type = "reviewer" if reviewer_decision else "user_input"
    snapshot.extend(
        CaseFactRecord(
            id=new_id(),
            org_id=case.org_id,
            ...
            effective_date=fact.effective_date,
            confirmation_status=decisions[fact.fact_key],
        )
        for fact in fact_proposals
    )
    return snapshot
