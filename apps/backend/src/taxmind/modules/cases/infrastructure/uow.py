...
class SqlAlchemyCasesUnitOfWork:
    def __init__(self, sessions: async_sessionmaker[AsyncSession]) -> None:
        self._sessions = sessions
        self._session: AsyncSession | None = None
        self.repository: SqlAlchemyCasesRepository | None = None

    async def __aenter__(self) -> SqlAlchemyCasesUnitOfWork:
...
class SqlAlchemyCasesUnitOfWorkFactory:
    def __init__(self, sessions: async_sessionmaker[AsyncSession]) -> None:
        self._sessions = sessions

    def __call__(self) -> SqlAlchemyCasesUnitOfWork:
        return SqlAlchemyCasesUnitOfWork(self._sessions)
